...
Be sure that your raspberry pi has enough cooling to do this job, it will be heavily used. It is also likely necessary to increase the available 
SWAP space for opencv and dlib to succesfully compile. SWAP can be increased by editing the file: `sudo nano /etc/dphys-swapfile` and changing
the value of `CONF_SWAPSIZE`. 1024 MB was found to work on a RPi 4 (i.e. `CONF_SWAPSIZE=1024`). 
